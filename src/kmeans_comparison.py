...
def _materialize_bag(client, X_bag):
    ...
    array on the client. ``to_delayed()`` yields one 2-D chunk per
    partition (nested ndarray on the chunk grid, flattened here), so only
    a handful of compact blocks travel over the network."""
